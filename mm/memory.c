				...
				struct zap_details *details)
{
	...
	//printk("lwt:%s, current is %d, %s,",__func__,current->pid, current->comm);
...
/* perform swap device frinedly unmap size */
...
next:
#ifdef CONFIG_MEM_UNMAP
        if (i < mem_unmap_unit_size * PAGE_SIZE)
        {
            i++;
        }
        else
		...
		unsigned long size)
{
	...
	printk("lwt:%s, size is %ld",__func__, size/4096);
	lru_add_drain();
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	ktime_t starttime, delta, doswappage_time;
	...
	doswappage_time = ktime_get();
    delta = ktime_sub(doswappage_time, starttime);
    duration = (unsigned long long) ktime_to_us(delta);//΢��
    // printk("lwt:%s doswappage_time %lld usecs, line is %d\n",__FUNCTION__, duration,__LINE__);
