...
static unsigned int shrink_page_list(struct list_head *page_list,
				     ...
				     bool ignore_references, unsigned long long *var)
{
	LIST_HEAD(ret_pages);
...
//    unsigned int while_count = 0;
   	ktime_t starttime, calltime,shpage_time, while_time, delta, beforetime, addswap_time, privatetime, unmaptime, wbtime, dirtytime, rmmaptime;
    unsigned long long duration=0,total_addswaptime=0, total_unmaptime=0, total_wbtime=0,total_dirtytime=0, total_privatetime=0,total_beforetime=0,total_rmmaptime=0;
    unsigned long long total_whiletime=0;
	// ktime_t starttime, delta, shpage_time;
	...
	while (!list_empty(page_list)) {
		...
		beforetime = ktime_get();
		delta = ktime_sub(beforetime,calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_beforetime = total_beforetime + duration;
		...
		wbtime = ktime_get();
		delta = ktime_sub(wbtime, calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_wbtime = total_wbtime + duration;
		// printk("lwt:%s writebacktime %lld usecs, line is %d\n",__FUNCTION__, duration, __LINE__);
		...
		addswap_time = ktime_get();
		delta = ktime_sub(addswap_time, calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_addswaptime = total_addswaptime + duration;
		//printk("lwt:%s addswaptime %lld usecs, line is %d\n",__FUNCTION__, duration, __LINE__);
	    ...
	    unmaptime = ktime_get();
		delta = ktime_sub(unmaptime,calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_unmaptime = total_unmaptime + duration;
		//printk("lwt:%s unmaptime %lld usecs, line is %d\n",__FUNCTION__, duration,__LINE__);
		...
		dirtytime = ktime_get();
		delta = ktime_sub(dirtytime,calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_dirtytime = total_dirtytime + duration;
		//printk("lwt:%s dirtytime %lld usecs, line is %d\n",__FUNCTION__, duration,__LINE__);
		...
		privatetime = ktime_get();
		delta = ktime_sub(privatetime, calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_privatetime = total_privatetime+ duration;
		//printk("lwt:%s privatetime %lld usecs, line is %d\n",__FUNCTION__, duration,__LINE__);
		...
		rmmaptime = ktime_get();
		delta = ktime_sub(rmmaptime, calltime);
		/*ת����΢��*/
		duration = (unsigned long long) ktime_to_us(delta);//΢��
		total_rmmaptime = total_rmmaptime + duration;
...
keep:
	...
	//printk("lwt:%s total_whiletime %d\n",__FUNCTION__, while_count);
	while_time = ktime_get();
	delta = ktime_sub(while_time, calltime);
	duration = (unsigned long long) ktime_to_us(delta);//΢��
	total_whiletime = total_whiletime+duration;
	//printk("lwt:%s, %lld %lld %lld %lld %lld %lld %lld %lld usecs\n",__FUNCTION__,total_beforetime, total_wbtime, total_addswaptime, total_unmaptime, total_dirtytime, total_privatetime, total_rmmaptime, total_whiletime);
	}

	// start_free = ktime_get();
	...
	shpage_time = ktime_get();

	delta = ktime_sub(shpage_time, starttime);
	duration = (unsigned long long) ktime_to_us(delta);//΢��
	//printk("lwt:%s, %lld %lld %lld %lld %lld usecs\n",__FUNCTION__, total_addswaptime, total_unmaptime, total_dirtytime, total_whiletime, duration);

	delta = ktime_sub(shpage_time, calltime);
	duration = (unsigned long long) ktime_to_us(delta);//΢��
	...
	//printk("lwt:%s shpage_time %lld usecs, nr_reclaimed %d, nr_swappage %d %d\n",__FUNCTION__, duration, nr_reclaimed, nr_swappage,nr_pageout);
	return nr_reclaimed;
}
		     ...
		     struct scan_control *sc, enum lru_list lru, unsigned long long *tar)
{
	...
	bool do_plug = false;
	ktime_t starttime, delta, shrinkinactive_time;
	unsigned long long duration=0;
	unsigned long long middle_time=0;
	starttime = ktime_get();
	...
	// isolate_start_time = ktime_get();

	nr_taken = isolate_lru_pages(nr_to_scan, lruvec, &page_list,
				     &nr_scanned, sc, lru);

	// isolate_time = ktime_get();
    // delta = ktime_sub(isolate_time, isolate_start_time);
    // duration = (unsigned long long) ktime_to_us(delta);//΢��
	...
	nr_reclaimed = shrink_page_list(&page_list, pgdat, sc, &stat, false, &middle_time);

	// shrinkpage_time = ktime_get();

    // delta = ktime_sub(shrinkpage_time, tmp_time);
    // duration = (unsigned long long) ktime_to_us(delta);//΢��
	*tar = middle_time;
	...
	shrinkinactive_time = ktime_get();
    delta = ktime_sub(shrinkinactive_time, starttime);
    duration = (unsigned long long) ktime_to_us(delta);//΢��
    //printk("lwt:%s shrinkinactive_time %lld usecs, percent %lld \n",__FUNCTION__, duration, 1000*middle_time/duration);
	return nr_reclaimed;
}
				 ...
				 struct lruvec *lruvec, struct scan_control *sc, unsigned long long *par)
{
    ...
    tmp = shrink_inactive_list(nr_to_scan, lruvec, sc, lru, &x_time);
	*par = x_time;
		       ...
		       bool *need_swapping)
{
	...
	reclaimed = shrink_page_list(&list, pgdat, sc, &stat, false,NULL);
...
static void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc, unsigned long long *lwt2)
{
	...
	ktime_t satrt_time, delta, shrinklruvec_time;
    unsigned long long duration=0, every_time=0, total_time=0;;
	...
	// scancount_time = ktime_get();
	// /*����ʱ��� ktime_sub(B,A) ����A-B��ʱ��� ��ʱ���ȡ��ʱ��������*/
    // delta = ktime_sub(scancount_time, satrt_time);
    //    /*ת����΢��*/
    // duration = (unsigned long long) ktime_to_us(delta);//΢��
    // printk("lwt:%s scancounttime %lld usecs, line is %d\n",__FUNCTION__, duration,__LINE__);
	...
	while (nr[LRU_INACTIVE_ANON] || nr[LRU_ACTIVE_FILE] ||
		...
		for_each_evictable_lru(lru) {
			if (nr[lru]) {
			    ...
			    //printk("lwt:%s nr_to_scan %ld, nr_reclaimed %ld, ratio %ld \n",__FUNCTION__, nr_to_scan, nr_reclaimed, 1000*nr_reclaimed/nr_to_scan);
				total_time=every_time+total_time;
				// printk("lwt:%s ETime %lld usecs, Ttime %lld usecs\n",__FUNCTION__, every_time,total_time);
			}
		}
    ...
    printk("lwt:%s, %s, proportional_reclaim %d nr_to_reclaim %ld nr_to_scan %ld nr_reclaimed is %ld ratio %ld %ld %ld\n",__FUNCTION__, current->comm, proportional_reclaim, nr_to_reclaim, total_scan, sc->nr_reclaimed, 1000*total_scan/nr_to_reclaim, 1000*(sc->nr_reclaimed)/nr_to_reclaim, 1000*(sc->nr_reclaimed)/total_scan);
	...
	shrinklruvec_time = ktime_get();
	delta = ktime_sub(shrinklruvec_time, satrt_time);
    duration = (unsigned long long) ktime_to_us(delta);//΢��
	*lwt2 = duration;
    // printk("lwt:%s shrinklruvec_time %lld msecs, line is %d\n",__FUNCTION__, duration,__LINE__);
	//printk("lwt:%s shrinklruvec_time %lld usecs, Middletime %lld usecs\n",__FUNCTION__, duration, total_time);
...
static void shrink_node_memcgs(pg_data_t *pgdat, struct scan_control *sc, unsigned long long *lwt1)
{
...
again:
	...
	shrink_node_memcgs(pgdat, sc, &tmp);
	...
	//printk("lwt: %s shrinknode_time %lld usecs per %lld, pid is %d, comm is %s, nr_to_reclaim is %d, %d\n",__FUNCTION__, duration, 1000*tmp/duration, current->pid, current->comm,sc->nr_to_reclaim,sc->nr_reclaimed);
}
...
static void shrink_zones(struct zonelist *zonelist, struct scan_control *sc)
{
	...
	direct_count++;
	printk("direct_count is %d",direct_count);
	/*
...
static int kswapd(void *p)
{
	...
	unsigned int kshrink_ret=0;
	kswapd_count++;
    printk("kswapd_count is %d",kswapd_count);
